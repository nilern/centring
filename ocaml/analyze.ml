...
and analyze_sf sf_name args =
  ...
  | Some "fn" ->
    let analyze_case = function
      | Stx (List [cond; body], _, _, _) -> (dnf (analyze cond), analyze body)
      | case -> raise (Invalid_case case) in
    (match args with
     | (Stx (Symbol name, _, _, _))::(Stx (Symbol formal, _, _, _))::cases ->
  ...
  | Some "def" ->
    (match args with
     | [(Stx (Int i, _, _, _)); (Stx (Symbol name, _, _, _)); val_expr] ->
