...
and value = Int of int
          ...
          | Char of char
          | Symbol of Symbol.t
          | FnClosure of Symbol.t * ast * (ast array * ast * env) Sequence.t
...
let () =
  ...
  match parse expr (String_seq.of_string "#(123 foo #f)") with
  | (Ok v, _) -> v 
    |> sexp_of_cexp
    |> Sexp.pp_hum Format.std_formatter
  | (Error msg, _) -> Out_channel.output_string stdout msg
