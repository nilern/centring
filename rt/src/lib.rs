...
impl GCState {
    ...
    #[inline(always)]
    fn rec_poll(&self, word_count: usize) -> bool {
        self.fromspace.capacity() - self.fromspace.len() >= word_count
    }

    fn alloc_blob(&mut self, byte_count: usize) -> ValueRef {
        unsafe {
    ...
    fn alloc_rec(&mut self, field_count: usize) -> ValueRef {
        unsafe {
